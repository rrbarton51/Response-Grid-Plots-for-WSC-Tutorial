...
#  for the area under the gap curve given x = eta_1, eta_2, gamma_1, gamma_2
...
#  and defines a function that does prediction using the GP object
  ...
  area = as.vector(TOMACSdataSAN[,"Area.Under.Progress.Curve"])
  ...
  mmSANarea <<- mleHetGP(DOE,area,covtype = "Gaussian", maxit = 1000)
  # define GP approximation function
  f <- function(x){
    area = predict(x = x, object = mmSANarea)$mean
    return(area)
  }
