...
inner <- function(innerDOE,outerd,innerd,ploc,spscale,rscale,cgray){
  ...
  if(innerd ==1){
    ...
    symbols(spscale*DOExyzS[1,1:ncol(DOExyz)]+ploc[1],spscale*DOExyzS[2,1:ncol(DOExyz)]+ploc[2],
            circles=1.2*psResp1,bty="n",xaxt="n",yaxt="n",
            xlab="",ylab="", bg=cgray,fg=cgray, inches=FALSE, add=TRUE)
    symbols(spscale*DOExyzS[1,1:ncol(DOExyz)]+ploc[1]+.9*(psResp1+psResp2),spscale*DOExyzS[2,1:ncol(DOExyz)]+ploc[2],
            circles=1.2*psResp2,bty="n",xaxt="n",yaxt="n",
            xlab="",ylab="", bg=cgray,fg=cgray, inches=FALSE, add=TRUE)
  } else{
    symbols(spscale*DOExyzS[1,1:ncol(DOExyz)]+ploc[1],spscale*DOExyzS[3,1:ncol(DOExyz)]+ploc[2],
            circles=.7*psResp1,bty="n",xaxt="n",yaxt="n",
            xlab="",ylab="", bg=cgray,fg=cgray, inches=FALSE, add=TRUE)
    symbols(spscale*DOExyzS[1,1:ncol(DOExyz)]+ploc[1]+.7*(psResp1+psResp2),spscale*DOExyzS[3,1:ncol(DOExyz)]+ploc[2],
            circles=.7*psResp2,bty="n",xaxt="n",yaxt="n",
            xlab="",ylab="", bg=cgray,fg=cgray, inches=FALSE, add=TRUE)
  }
  ...
  if(innerd == 1 | innerd == 2){
    text(((min(x0)+max(x0))/2)+.1*epsx,min(y0)-.4*epsy,plotnames[outerd+1],cex=1.5*tscale)
  }
  if(innerd == 2){
    text(max(x1)+.4*epsx+(psResp1+psResp2),+min(y0)+.5*epsy,plotnames[outerd+2],srt=90,cex=1.5*tscale)
  }
  if(innerd == 3){
