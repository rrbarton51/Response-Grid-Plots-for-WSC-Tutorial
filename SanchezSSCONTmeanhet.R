...
#  and defines a function that does prediction using the GP object
  ...
  opt_gap = as.vector(TOMACSdataSSCONT[,"Final.Relative.Optimality.Gap"])
  ...
  mmSSCONTgap <<- mleHetGP(DOE,opt_gap,covtype = "Gaussian", maxit = 1000)
  # define GP approximation function
  f <- function(x){
    gap = predict(x = x, object = mmSSCONTgap)$mean
    return(gap)
  }
