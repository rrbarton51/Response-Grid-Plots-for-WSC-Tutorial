...
#  and construct a response-scaled design plot on 
#  a factorial grid with number of levels nlev[1] ... nlev[d] and 
#  x limits xlo[1] ... xlo[d] and xhi[1] ... xhi[d]
# Variable names must be provided in first row of csv, 
# No run id column allowed
...
rgpData <- function(dataname,fname){
  ...
  DOE = data.matrix(fac.design(factor.names=xlab[1:d],nlevels=nlev,randomize=FALSE))
  ...
  colnames(xDOE) = xlab
  xDOE = as.data.frame(xDOE)
  
  # assign f value for each row in xDOE - begin with values 1/4 of
  # range less than min (will remain for any conditions missing from DOEdata)
  fdatamin=min(fdata);fdatamax=max(fdata);fdatarange=fdatamax-fdatamin
  fdatazero <<- fdatamin - .5*fdatarange # needed in outer and inner
  resp = rep(fdatazero,nrow(xDOE)) # values initially set to missing


  #for each row of DOEdata, put corresponding f value in matching row of xDOE
  # requires package prodlim
  library(prodlim)
  matches=row.match(x = DOEdata[,1:d], table = xDOE[,1:d])
  for (row in 1:nrow(DOEdata)) {
    resp[matches[row]] = fdata[row]
  }
  
  #order column variables based on number of levels
...
outer <- function(unscaledDOE,outerd,innerd){
  ...
  # for rgpData, initially set all responses to min
  ...
  sResp=ifelse(resp==fdatazero,0,sResp) # change missing values to zero
